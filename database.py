...
def add_product(user_id, name, desc, price, cat, img, stock):
    try: products_col.insert_one({"id": f"P-{uuid.uuid4().hex[:6]}", "u_id": user_id, "name": name, "description": desc, "price": float(price), "category": cat, "image_url": img, "stock": int(stock), "created_at": datetime.now(), "ratings_sum": 0, "ratings_count": 0, "rated_ips": {}}); return True
    except: return False
...
# 🔒 الدالة الأمنية الجديدة: حساب الأسعار عبر السيرفر حصراً
...
def add_package(name, price, max_products, features): packages_col.insert_one({'name': name, 'price': price, 'max_products': int(max_products), 'features': features})
