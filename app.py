...
# إرسال إشعارات التلجرام للإدارة
...
# حقن المتغيرات العامة (شعار المنصة) لكل القوالب
...
@app.route('/sw.js')
def service_worker(): 
    return Response("self.addEventListener('install', (e) => { console.log('[TajerGo PWA] Installed'); }); self.addEventListener('fetch', (e) => {});", mimetype="application/javascript")

# 🛡️ الحماية: حساب السلة يتم في الخادم فقط (Clean Code Security)
...
def checkout(slug):
    ...
    # الدالة الآمنة لإنشاء الطلب في الخادم
    order_id, real_total, secure_cart, discount_info = database.create_secure_order(user.get('id'), data['name'], data['phone'], address, payment, data['cart'], coupon_code)
    ...
    whatsapp_link = f"https://wa.me/{settings.get('whatsapp', '')}?text={urllib.parse.quote(msg)}"
    return jsonify({"whatsapp_url": whatsapp_link})
...
def rate_product_api():
    try:
        ...
        if product:
            ...
            if old_stars: new_sum, new_count = c_sum - old_stars + stars, cr
            else: new_sum, new_count = c_sum + stars, cr + 1
...
def dashboard():
    ...
    is_super_admin = (session.get('store_slug') == 'admin-store')
    
    if request.method == 'POST':
        action = request.form.get('action')
        
        # إجراءات المنتجات والطلبات
        ...
        # إجراءات الكوبونات
            ...
            database.add_coupon(session['user_id'], request.form.get('code'), request.form.get('discount'))
            flash("تم إنشاء الكوبون", "success")
        elif action == 'delete_coupon': 
            database.delete_coupon(request.form.get('coupon_id'), session['user_id'])
            flash("تم حذف الكوبون", "danger")
        ...
        # الإعدادات
            ...
            # حفظ شعار المنصة للمدير العام فقط
            if is_super_admin and request.form.get('platform_logo'): 
        ...
        # إجراءات المدير العام
            ...
            if database.edit_merchant_info(request.form.get('user_id'), request.form.get('new_slug', '').strip(), request.form.get('new_package', 'أساسية')): 
    ...
    # الإحصائيات (Clean Data Processing)
    def clean_total(t):
    ...
    # تمرير كل المتغيرات للقالب
    return render_template('dashboard.html', 
                           products=products, 
                           coupons=coupons, 
                           settings=settings, 
                           orders=orders, 
                           stats={"total_orders": len(orders), "total_revenue": net_sales, "status_counts": status_counts}, 
                           merchants=(database.get_all_users() if is_super_admin else []), 
                           packages=database.get_packages(), 
                           current_user_data=database.users_col.find_one({'id': session['user_id']}), 
